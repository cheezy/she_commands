defmodule SheCommandsWeb.UserAuth do
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      SheCommandsWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def on_mount(:ensure_coach_or_admin, _params, session, socket) do
    socket = mount_current_scope(socket, session)

    if socket.assigns.current_scope &&
         socket.assigns.current_scope.user.role in [:coach, :admin] do
      {:cont, socket}
    else
      socket =
        socket
